...
void exit_with_help(){
    ...
    cerr << "-s epsilon: stop criterion (default 1e-6)"<<endl;
    exit(0);
}
    ...
    for(i=1;i<argc;i++){
        ...
        switch(argv[i-1][1]){
            ...
            case 't': param.alpha = atof(argv[i]);
                      break;
            case 's': param.outer_eps = atof(argv[i]);
                      break;
            default:
...
void runHybrid(Problem* prob, Param param,double* solu_eigvec,double& solu_eigval){
    set_prob(prob);
    double eta = param.eta;
    double old_eta = eta;
    double alpha = param.alpha;
    cerr<<"alpha="<<alpha<<endl;
    double epsilon = 1e-6;//param.outer_eps; 
	cerr<<param.outer_eps<<endl<<endl;
    int m = prob->m;
    int n = prob->n;
    double* b = prob->b;
    ...
    prob->neg_grad_largest_ev(y,0.0,1e-8,new_k,new_us,new_eigenvalues,0); 
    double C_spectral = fabs(new_eigenvalues[0]);
    prob->neg_grad_largest_ev(y,0.0,1e-8,1,new_us,new_eigenvalues,1);
    ...
    double* solu_thres = new double[prob->n];
...
//	    for (int ii = 0;ii<new_k;ii++)		    cerr<<new_eigenvalues[ii]<<" "; cerr<<endl;
	    // push new coordinates into V
            clock_t t_push = clock();
	    for (int j = 0;j < new_k;j++){
                double eigenvalue = new_eigenvalues[j];
                new_u = new_us + j*n;
                if ( eigenvalue > 1e-3){ // will probably cause some problem...
...
//	    cerr<<"real_new_k="<<real_new_k<<endl;
	    ...
	    cerr<<" "<<(double)(clock()-t_push)/CLOCKS_PER_SEC<<" " ;
...
//	    num_rank1 = min(num_rank1,max_rank);
	...
	clock_t t_nonconvex = clock();	
	prob->non_convex_solver(V, num_rank1, eta, y);
	
	cerr<<" "<<(double)(clock()-t_nonconvex)/CLOCKS_PER_SEC<<" " ;
...
//	cerr<<"step size="<<alpha<<endl;
	...
	dobj = -dot(y,b,m);    
        ...
        clock_t t_yt = clock();
	running_time += ((double)(t_yt - iter_start))/CLOCKS_PER_SEC ;
        
	cerr<<t_eig<<" "<<t_inner<<endl;	
	old_eta = eta;
	// update eta
	//eta *= 2;
	double max_eta = 1000;
...
*/
	...
	while (V.size()>num_rank1){
	    V.pop_back();
	    num_rank1_capacity--;
	}
	    
	largest_ev_solution(V, n, 1e-8, solu_eigvec, solu_eigval);
// for max-cut problem only    
    double lambdah = sqrt(solu_eigval);
    for (int i=0;i<prob->n;i++){
	    if(lambdah*solu_eigvec[i]<0) 
	    { 
		    solu_thres[i] = -1;
	    }
	    else{
    ...
    for (int i=0;i<prob->n;i++){
        double tmp = 0.0;
        for (SparseVec::iterator vit = prob->C[i]->begin();vit != prob->C[i]->end(); vit++){
            tmp += vit->second * solu_thres[vit->first];
        }
        obj_thres += tmp*solu_thres[i];
    }
    cerr<<"yt iter="<<yt_iter<<", t="<<running_time<<", num_rank1="<<num_rank1<<", eta="<<eta<<", obj="<<setprecision(10)<<obj<<", dobj="<<dobj<<", pinf="<< pinf <<", dinf="<<dinf<<", Gap="<<duality_gap<<" obj after thres="<<obj_thres<<endl;
    if (max(max(pinf, dinf),duality_gap) < param.outer_eps)
	    break;
...
int main(int argc, char** argv){
    ...
    double* solu_eigvec = new double[prob->n];
   double solu_eigval = 0; 
    runHybrid(prob,param,solu_eigvec,solu_eigval);
